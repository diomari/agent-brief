...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("onboard", {
    description: "Generate or refresh a compact project onboarding context",
    handler: async (args, ctx) => {
      try {
        ...
        if (exists && !options.refresh && !options.dryRun) {
          ctx.ui.notify(`${displayPath} already exists. Run /onboard --refresh to regenerate.`, "warning");
          await pi.sendUserMessage(generateKickoffPrompt(displayPath, options, false));
          return;
        }
        ...
        if (options.dryRun) {
          ctx.ui.notify(
            `Dry run: ${options.full ? "full" : "compact"} brief is ${markdown.split("\n").length} lines for ${displayPath}.`,
            "info",
          );
        } else {
          ...
          ctx.ui.notify(`${exists ? "Refreshed" : "Generated"} ${displayPath}.`, "success");
        }

        await pi.sendUserMessage(generateKickoffPrompt(displayPath, options, !options.dryRun));
      } catch (error) {
